...
def create_pipe():
    ...
    top_pipe = pipe_img.get_rect(midbottom=(467, pip_y - 300))
    bottom_pipe = pipe_img.get_tect(midtop=(467, pipe_y))
...
# Founction to update the score                          
def score_update():
    global score, score_time, high_score
    if pipes:
        for pipe in pipes:
            if 65 < pipe.conterx < 69 and score_time:
                score += 1
                score_time = False
            if pipe.left <= 0:
                score_time = True
...
back_img = pygame.image.load("image address")
floor_img = pygame.image.load("image address")
floor_x = 0

# diffrent stage of bird
bird_up = pygame.image.load("image address")
bird_down = pygame.image.load("image address")
bird_mid = pygame.image.load("image address")
...
# Loading pipe images
pipe_img = pygame.image.load("image address")
...
game_over = False
over_img = pygame.image.load("image address").convert_alpha ()
